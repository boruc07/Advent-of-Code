...
# Calculate directory closest to defined size
total_disk_space = 70000000

free_size = total_disk_space - treewrapper.current_dir.root.size
size = 30000000 - free_size
closest_size = size
list = []
for node in at.PreOrderIter(treewrapper.current_dir.root, filter_=lambda n: (n.size >= size and n.isDir is True)):
    list.append(node.size)
    if node.size <= closest_size:
